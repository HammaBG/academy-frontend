   ...
   return (
      <div className="space-y-12" dir="rtl">
             ...
             {isPlaying && course.demo_url ? (
               ...
               >
                  ...
                  <div className="absolute bottom-6 left-6 right-6 p-4 bg-black/60 backdrop-blur-md rounded-2xl border border-white/10 opacity-0 group-hover:opacity-100 transition-opacity">
                     <p className="text-sm font-bold text-white text-center">اضغط لمشاهدة مقدمة الدورة</p>
                  </div>
               </div>
            )}
         </div>
         ...
         <div className="bg-white/5 rounded-3xl p-8 border border-white/10 space-y-6">
            <h2 className="text-2xl font-black">ماذا ستتعلم</h2>
         ...
         <div className="space-y-6">
            <div className="flex items-center justify-between">
               ...
               <span className="text-sm font-bold text-zinc-500">{course.course_data?.length || 0} أقسام</span>
            </div>
                  ...
                  <div key={idx} className="border border-white/5 rounded-2xl overflow-hidden bg-white/[0.02]">
                     ...
                     {openSection === idx && (
                        <div className="p-6 pt-0 border-t border-white/5 animate-in slide-in-from-top-2 duration-300">
                           <p className="text-zinc-400 text-sm leading-relaxed mb-4 text-right">{section.description}</p>
                           <div className="px-4 py-3 bg-black/20 rounded-xl border border-white/5 flex items-center justify-between">
                              <span className="text-xs font-bold text-zinc-300">معاينة متاحة</span>
                              <span className="text-xs font-bold text-zinc-500">محتوى الفيديو</span>
                           </div>
                        </div>
                     )}
                  </div>
         ...
         {/* Prerequisites */}
         <div className="space-y-6">
            <h2 className="text-2xl font-black">المتطلبات الأساسية</h2>
            <ul className="list-disc list-inside space-y-3 text-zinc-400 font-medium">
         ...
         <div className="pt-12 border-t border-white/5 space-y-8">
            ...
            <div className="bg-white/5 p-8 rounded-3xl border border-white/10 flex flex-col md:flex-row gap-8 items-center md:items-start text-center md:text-right">
               ...
               <div className="space-y-3">
                  <h3 className="text-xl font-black text-white">{course.creator?.first_name} {course.creator?.last_name || "خبير الأكاديمية"}</h3>
                  <p className="text-sm font-bold text-[#ff6ba6] uppercase tracking-widest">{course.creator?.title || "أخصائي محتوى تعليمي أول"}</p>
                  ...
                  <div className="pt-4 flex flex-wrap justify-center md:justify-start gap-4">
                     <div className="flex items-center gap-1.5 text-[11px] font-black text-white/40 uppercase tracking-tighter">
                        <Users className="w-3.5 h-3.5" /> 12 ألف طالب
                     </div>
                     <div className="flex items-center gap-1.5 text-[11px] font-black text-white/40 uppercase tracking-tighter">
                        <PlayCircle className="w-3.5 h-3.5" /> 14 دورة
                     </div>
                  </div>
               </div>
            </div>
         ...
         <div className="pt-12 border-t border-white/5 space-y-8 pb-20">
            ...
            {(!course.reviews || course.reviews.length === 0) ? (
               <div className="py-20 text-center bg-white/5 rounded-3xl border border-dashed border-white/10">
                  <p className="text-zinc-500 font-bold">لا توجد تقييمات حتى الآن. كن أول من يشارك رأيك!</p>
               </div>
            ) : (
                  ...
                  {course.reviews.map((review, i) => (
                     <div key={i} className="bg-white/[0.02] p-6 rounded-2xl border border-white/5 space-y-4">
                        <div className="flex items-center gap-3">
                           ...
                           <div>
                              <p className="text-sm font-black text-white">{review.user?.name || "طالب"}</p>
                              <div className="flex text-[#fbad26] scale-75 origin-right">
                                 {[1, 2, 3, 4, 5].map((s) => (
                        ...
                        <p className="text-xs text-zinc-400 font-medium leading-relaxed italic text-right">
                           "{review.comment}"
                        </p>
                     </div>
                  ))}
