...
function BlobCard({ number, label, imageUrl, className = "", aspectClass = "aspect-square", bgColor }: BlobCardProps) {
  ...
  return (
    <div
      ...
      className={`w-full rounded-[1.5rem] p-4 flex flex-col justify-center items-center text-center shadow-sm hover:shadow-md hover:-translate-y-1 transition-all duration-500 group ${aspectClass} ${className} ${bgColor ? "text-white border-0" : "bg-surface border border-border/40"
        }`}
    >
      <span
        className={`text-xl sm:text-2xl font-black transition-transform duration-300 group-hover:scale-105 ${bgColor ? "text-white" : "bg-gradient-to-br from-brand-primary to-[#ff6ba6] bg-clip-text text-transparent"
          }`}
      >
        {number}
      </span>
      <span
        className={`text-[10px] sm:text-xs font-bold mt-1.5 tracking-wide leading-tight px-1 ${bgColor ? "text-white/90" : "text-text-secondary"
          }`}
      >
        {label}
      </span>
        ...
        <div className="hidden lg:flex lg:col-span-6 justify-center lg:justify-end">
          ...
          <div className="relative h-[650px] w-full max-w-[580px] mx-auto overflow-hidden flex gap-4 items-start lg:-mt-20 z-0">
            ...
            <div className="w-1/3 flex flex-col gap-4 animate-marquee-up hover:[animation-play-state:paused]">
              ...
              <BlobCard imageUrl="/Ossos/h2.jpeg" aspectClass="aspect-square" />
            ...
            <div className="w-1/3 flex flex-col gap-4 animate-marquee-down hover:[animation-play-state:paused]">
              ...
              <BlobCard number="+50" label="كادر طبي" aspectClass="aspect-square" bgColor="#37c597" />
              {/* Set 2 */}
              <BlobCard number="+10" label="شركاء الأكاديمية" aspectClass="aspect-square" bgColor="#4da1f7" />
              <BlobCard imageUrl="/Ossos/ca.jpeg" aspectClass="aspect-square" />
              <BlobCard number="+50" label="كادر طبي" aspectClass="aspect-square" bgColor="#37c597" />
            ...
            <div className="w-1/3 flex flex-col gap-4 animate-marquee-up hover:[animation-play-state:paused]">
              ...
              <BlobCard imageUrl="/Ossos/h4.jpeg" aspectClass="aspect-square" />
